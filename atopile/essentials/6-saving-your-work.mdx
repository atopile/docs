...
If you're not yet familiar with `git`, check out the short [`git` guide](../guides/git.md) for a general overview of the workflow.
...
<Note>
    ...
    This includes the current directory if you use `git add .` — but be careful! This is often a trap, and you end up adding a lot of unrelated files. At the start of a project, however, it often makes sense.
</Note>
...
atopile's project template includes basic CI workflows to check your design, build manufacturing data you can order with, and run testing.
