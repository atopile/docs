...
<Note>
    This tutorial continues from the [quickstart](../quickstart) guide.

    If you haven't already, complete it, and then come back here.
</Note>
With the explanation of `ato` code and the [build process](#build-process), it should be a little more clear what's happening now in the [quickstart](../quickstart) project.
